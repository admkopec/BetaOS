    ...
    required init?(pci: PCI) {
        ...
        while ip > 0 {
            if ip & 1 == 1 {
                ...
                if let device = SATADevice(port: Port, PortNumber: i) {
...
struct SATADevice: Module, Disk {
    ...
    init?(port: UnsafeMutablePointer<HBA_PORT>, PortNumber: Int) {
        Port = port
        Slot = 0
    ...
    func CommitCommand(slot: Int) -> Bool { // Needs a little bit more refining
        if Port.pointee.cmd & 1 != 1 {
            Log("Starting command processing...", level: .Debug)
            Port.pointee.cmd |= (1 << 0) | (1 << 4)
        }
    ...
    fileprivate mutating func Rebase(PortNumber: Int) {
        ...
        addr64 = NewBasePhys + 32768 + (UInt64(PortNumber) << 8)
        ...
        for i in 0 ... 31 {
            CommandHeader.advanced(by: i).pointee.prdtl = 5 // or 8
