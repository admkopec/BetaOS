...
cpuid_set_cache_info( i386_cpu_info_t * info_p )
{
    ...
    /* Get processor cache descriptor info using leaf 2. */
     ...
     * Most processors should support implementation of this flavor of CPUID.
    ...
    for (index = 0; cpuid_deterministic_supported; index++) {
        ...
        if (type != Lnone) {
             ...
             * by the page offset.
             */
            // We're not going to use a VM now, but it's good to read all values
...
i386_cpu_info_t*
cpuid_info(void) {
    /* Set-up the cpuid_info stucture */
    if (cpuid_cpu_infop == NULL) {
        //Parse_boot_argn("-cpuid", &cpuid_dbg, sizeof(cpuid_dbg));
        cpuid_set_info();
...
cpuid_init_vmm_info(i386_vmm_info_t *info_p) {
    ...
    if (true == strcmp(info_p->cpuid_vmm_vendor, CPUID_VMM_ID_VMWARE)) {
        /* VMware identification string */
        info_p->cpuid_vmm_family = CPUID_VMM_FAMILY_VMWARE;
    } else if (true == strcmp(info_p->cpuid_vmm_vendor, CPUID_VMM_ID_PARALLELS)) {
...
cpuid_features(void) {
    ...
    __attribute__((unused))char	fpu_arg[20] = { 0 };
    cpuid_info();
    if (!checked) {
        /* check for boot-time fpu limitations */ // We're not using any limitations now
        /*if (Parse_boot_argn("_fpu", &fpu_arg[0], sizeof (fpu_arg))) {
